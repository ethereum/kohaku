...
export const myDepositsWithAssetSelector = createSelector(
  [myDepositsSelector, entrypointDepositSelector, poolsSelector],
  (myDeposits, entrypointDepositsMap, poolsMap): Map<Precommitment, IDepositWithAsset> => {
    const depositsWithAssets = Array.from(myDeposits)
      .map(([precommitment, deposit]) => {
        ...
        return [precommitment, { ...deposit, assetAddress: pool.asset }] as const;
      })
...
export const myDepositsBalanceSelector = createSelector(
  [myWithdrawalsSelector, myRagequitsSelector, myDepositsWithAssetSelector],
  (withdrawalsMap, ragequitsMap, myDeposits): Map<Precommitment, IDepositWithBalance> => {
    const depositsByPrecommitment = Array.from(myDeposits.entries())
      .map(([precommitment, deposit]): [Precommitment, IDepositWithBalance] => {
        ...
        if (!depositData) {
          return [precommitment, { ...deposit, assetAddress: 0n, index: 0, balance: 0n }] as const;
        }

        const ragequit = ragequitsMap.get(depositData.precommitment);

        if (ragequit) {
          return [precommitment, { ...depositData, balance: 0n }] as const;
        }
        ...
        return [precommitment, { ...depositData, balance: depositData.value - totalWithdrawn }] as const;
      });
...
export const depositsToSpendSelector = createSelector(
  [
...
export const specificAssetsBalanceSelector: SpecificAssetBalanceSelectorFn = createSelector(
  [
