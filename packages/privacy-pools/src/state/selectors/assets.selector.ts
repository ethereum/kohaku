...
import { myPoolsSelector } from "./pools.selector";
import { assetSelector, poolsSelector } from "./slices.selectors";

export const areAssetsSyncedSelector = createSelector(
  [myPoolsSelector, assetSelector],
  (myPools, assets): boolean => {
    const uniqueAssetAddresses = new Set(myPools.map(({ asset }) => asset));
