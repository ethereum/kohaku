...
export async function getPublicKey(config) {
    ...
    try {
        const ret = falcon._zknox_crypto_sign_keypair_from_seed(pkPtr, skPtr, seedPtr, seed.length);
        if (ret !== 0) throw new Error("Falcon-512 keygen failed (code " + ret + ")");

        const publicKey = new Uint8Array(falcon.HEAPU8.subarray(pkPtr, pkPtr + PK_LEN));
