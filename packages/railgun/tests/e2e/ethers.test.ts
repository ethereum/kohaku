...
import { type AnvilInstance, defineAnvil } from "../utils/anvil";
...
describe("Railgun E2E Flow", () => {
  ...
  it("should complete full shield -> transfer -> unshield flow", async () => {
    ...
    const aliceInitialEthBalance = await getETHBalance(provider, alice.address);
    ...
    console.log(
      `Alice initial ETH balance: ${aliceInitialEthBalance.toString()}`
    );
    ...
    const railgunCode = await provider.getCode(
      RAILGUN_CONFIG_BY_CHAIN_ID[chainId]!.RAILGUN_ADDRESS!
    );
    const relayAdaptCode = await provider.getCode(
      RAILGUN_CONFIG_BY_CHAIN_ID[chainId]!.RELAY_ADAPT_ADDRESS!
    );
    const wethCode = await provider.getCode(
      RAILGUN_CONFIG_BY_CHAIN_ID[chainId]!.WETH!
    );

    console.log(
      `RAILGUN contract code length: ${railgunCode.length} (${
        railgunCode === "0x" ? "MISSING!" : "exists"
      })`
    );
    console.log(
      `RELAY_ADAPT contract code length: ${relayAdaptCode.length} (${
        relayAdaptCode === "0x" ? "MISSING!" : "exists"
      })`
    );
    console.log(
      `WETH contract code length: ${wethCode.length} (${
        wethCode === "0x" ? "MISSING!" : "exists"
      })`
    );
    ...
    console.log(
      `Receipt status: ${receipt?.status} (1 = success, 0 = failure)`
    );
    ...
    if (receipt?.logs && receipt.logs.length > 0) {
      ...
      receipt.logs.forEach((log, idx) => {
        console.log(
          `  Log ${idx}: address=${log.address}, topics=${log.topics.length}`
        );
      });
    } else {
    ...
    console.log(
      `Alice balance after transfer: ${aliceBalanceAfterTransfer.toString()}`
    );
    ...
    const charlieBalanceWETHBeforeUnshield = await wethContract.balanceOf(
      charlie.address
    );

    console.log(
      `Charlie's WETH balance before unshield: ${charlieBalanceWETHBeforeUnshield.toString()} ${formatEther(
    ...
    const unshieldReceipt = await provider.getTransactionReceipt(
      unshieldTxHash
    );
    ...
    const charlieBalanceWETHAfterUnshield = await wethContract.balanceOf(
      charlie.address
    );

    console.log(
      `Charlie's WETH balance after unshield: ${charlieBalanceWETHAfterUnshield.toString()} ${formatEther(
        charlieBalanceWETHAfterUnshield
      )}`
    );
