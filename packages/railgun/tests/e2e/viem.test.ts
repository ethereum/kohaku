...
describe("Railgun E2E Flow (Viem)", () => {
  ...
  beforeAll(async () => {
    ...
    const aliceAccount = privateKeyToAccount(
      TEST_ACCOUNTS.alice.privateKey as `0x${string}`
    );
    const bobAccount = privateKeyToAccount(
      TEST_ACCOUNTS.bob.privateKey as `0x${string}`
    );
  ...
  it("should complete full shield -> transfer -> unshield flow", async () => {
    ...
    const charlieBalanceWETHBeforeUnshield = await wethContract.read.balanceOf([
      charlieAddress,
    ]);
    ...
    const charlieBalanceWETHAfterUnshield = await wethContract.read.balanceOf([
      charlieAddress,
    ]);
