...
export type CreateTransferTxFn = (
  token: Address,
  value: bigint,
...
export const makeCreateTransfer = async ({
  network,
  getTrees,
  getTransactNotes,
}: CreateTransferContext): Promise<CreateTransfer> => {
  const transfer: CreateTransferTxFn = async (
    token,
    value,
    receiver,
