...
export type CreateUnshieldTxFn = (
  token: Address,
  value: bigint,
  receiver: Address,
  minGasPrice?: bigint
) => Promise<TxData>;
export type CreateUnshieldNativeTxFn = (
  value: bigint,
  receiver: string,
  minGasPrice?: bigint
) => Promise<TxData>;
export type CreateUnshield = {
  unshield: CreateUnshieldTxFn;
  unshieldNative: CreateUnshieldNativeTxFn;
};
...
export const makeCreateUnshield = async ({
  network,
  getTrees,
  getTransactNotes,
}: CreateUnshieldContext): Promise<CreateUnshield> => {
  const unshield: CreateUnshieldTxFn = async (
    token,
    value,
    receiver,
    minGasPrice = BigInt(0)
  ) => {
    const { notesIn, notesOut } = await getTransactNotes(
      token,
      value,
      receiver
    );
  ...
  const unshieldNative: CreateUnshieldNativeTxFn = async (
    value,
    receiver,
    minGasPrice = BigInt(0)
  ) => {
    const { notesIn, notesOut, nullifiers } = await getTransactNotes(
      network.WETH,
      value,
      network.RELAY_ADAPT_ADDRESS,
      true
    );
    const unwrapTxData = createTx(
      network.RELAY_ADAPT_ADDRESS,
      RELAY_ADAPT_INTERFACE.encodeFunctionData("unwrapBase", [0])
    );
    const ethTransfer = [
      {
    ...
    const transferTxData = createTx(
      network.RELAY_ADAPT_ADDRESS,
      RELAY_ADAPT_INTERFACE.encodeFunctionData("transfer", [ethTransfer])
    );
    const actionData = {
    ...
    const nullifiers2D: string[][] = nonEmptyNullifiers.map((arr) =>
    ...
    const data = RELAY_ADAPT_INTERFACE.encodeFunctionData("relay", [
      allInputs,
      actionData,
    ]);
