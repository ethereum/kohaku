...
export type CreateShieldTxFn = (
  token: Address,
  value: bigint
) => Promise<TxData>;
export type CreateShieldNativeTxFn = (value: bigint) => Promise<TxData>;
export type CreateShieldMultiTxFn = (
  tokens: Address[],
  values: bigint[]
) => Promise<TxData>;
export type CreateShield = {
  shield: CreateShieldTxFn;
  shieldNative: CreateShieldNativeTxFn;
  shieldMulti: CreateShieldMultiTxFn;
};
...
export const makeCreateShield = async ({
  network,
  master,
  viewing,
  signer,
}: CreateShieldContext): Promise<CreateShield> => {
  ...
  const createShieldRequest = async (
    token: Address,
    value: bigint
  ): Promise<ShieldRequestStruct> => {
  ...
  const shieldNative: CreateShieldNativeTxFn = async (value) => {
    ...
    const wrapTxData = createTx(
      network.RELAY_ADAPT_ADDRESS,
      RELAY_ADAPT_INTERFACE.encodeFunctionData("wrapBase", [value])
    );
    const shieldTxData = createTx(
      network.RELAY_ADAPT_ADDRESS,
      RELAY_ADAPT_INTERFACE.encodeFunctionData("shield", [[request]])
    );
    const data = RELAY_ADAPT_INTERFACE.encodeFunctionData("multicall", [
      true,
      [wrapTxData, shieldTxData],
    ]);
  ...
  const shieldMulti: CreateShieldMultiTxFn = async (tokens, values) => {
    ...
    const wrapTxData = createTx(
      network.RELAY_ADAPT_ADDRESS,
      RELAY_ADAPT_INTERFACE.encodeFunctionData("wrapBase", [nativeValue])
    );
    const shieldTxData = createTx(
      network.RELAY_ADAPT_ADDRESS,
      RELAY_ADAPT_INTERFACE.encodeFunctionData("shield", [requests])
    );
    const data = RELAY_ADAPT_INTERFACE.encodeFunctionData("multicall", [
      true,
      [wrapTxData, shieldTxData],
