...
export type GetTransactNotesFn = (token: Address, value: bigint, receiver: string, getNullifiers?: boolean) => Promise<TransactNotes>;
export type GetUnspentNotesFn = (token: Address) => Promise<Note[][]>;
export type GetNotes = { getAllNotes: GetAllNotesFn, getTransactNotes: GetTransactNotesFn, getUnspentNotes: GetUnspentNotesFn };
...
export const makeGetNotes = async ({ notebooks, getTrees, spending, viewing }: GetNotesContext): Promise<GetNotes> => {
    ...
    const getTransactNotes: GetTransactNotesFn = async (token, value, receiver, getNullifiers = false) => {
