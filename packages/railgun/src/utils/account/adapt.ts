...
export const toActionDataTuple = (
  a: ActionData
): [
  string,
  boolean,
  bigint | number | string,
  Array<[string, string, bigint | number | string]>
] => {
  const callsTuple = a.calls.map(
    (c): [string, string, bigint | number | string] => [
      c.to,
      c.data ?? "0x",
      c.value ?? 0n,
    ]
...
export const getAdaptParamsHash = (
  ...
  const encoded = coder.encode(
    [
      "bytes32[][]",
      "uint256",
      "tuple(bytes31,bool,uint256,tuple(address,bytes,uint256)[])",
    ],
