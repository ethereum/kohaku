...
const TOKEN_ADDRESS: string = ByteUtils.formatToByteLength(config.contracts.rail, ByteLength.Address, true);
...
describe('shield-note', () => {
  ...
  it('Should validate length of random parameter', () => {
    ...
    mpk = getRandomScalar();
    expect(() => new ShieldNoteERC20(mpk, ByteUtils.randomHex(15), 1000n, TOKEN_ADDRESS)).to.throw(msg);
    expect(() => new ShieldNoteERC20(mpk, ByteUtils.randomHex(17), 1000n, TOKEN_ADDRESS)).to.throw(msg);
    expect(new ShieldNoteERC20(mpk, ByteUtils.randomHex(16), 1000n, TOKEN_ADDRESS)).to.be.an.instanceOf(
      ShieldNote,
    );
  });
