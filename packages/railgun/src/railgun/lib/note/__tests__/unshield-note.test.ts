...
describe("unshield-note", () => {
  it("Should get unshield fee and amount from value", () => {
    ...
    expect(UnshieldNote.getAmountFeeFromValue(100n, 25n)).to.deep.equal({
      fee: 0n,
      amount: 100n,
    });
    expect(UnshieldNote.getAmountFeeFromValue(399n, 25n)).to.deep.equal({
      fee: 0n,
      amount: 399n,
    });
    expect(UnshieldNote.getAmountFeeFromValue(400n, 25n)).to.deep.equal({
      fee: 1n,
      amount: 399n,
    });
  });
