...
export class PoseidonMerkleAccumulatorContract extends EventEmitter {
  ...
  constructor(
    ...
    this.eventTopic = this.contract
      .getEvent("AccumulatorStateUpdate")
  ...
  async validateMerkleroot(tree: number, root: string): Promise<boolean> {
    try {
      ...
      const err = new Error("Failed to validate V3 Poseidon merkleroot", {
        cause,
      });
  ...
  private async scanAllUpdateV3Events(
    ...
    try {
      ...
      if (!(cause instanceof Error)) {
        throw new Error("Non-error was thrown during scanAllUpdateV3Events", {
          cause,
        });
      }
  ...
  async getHistoricalEvents(
    ...
    while (currentStartBlock < latestBlock) {
      ...
      const allUpdateV3EventLogs = await this.scanAllUpdateV3Events(
        currentStartBlock,
        endBlock
      );
