...
export class V3Events {
  ...
  private static formatTransactCommitments(
    ...
    return commitmentCiphertexts.map((commitmentCiphertext, index) => {
      return {
        ...
        hash: ByteUtils.formatToByteLength(
          commitmentHashes[index],
          ByteLength.UINT_256
        ),
  ...
  static formatCommitmentCiphertext(commitmentCiphertext: {
    ...
    const strippedCiphertext = ByteUtils.strip0x(
  ...
  static formatUnshieldEvent(
    ...
    return {
      txid: ByteUtils.formatToByteLength(transactionHash, ByteLength.UINT_256),
      toAddress: ByteUtils.formatToByteLength(
  ...
  static formatNullifiedEvents(
    ...
    for (const nullifierHash of nullifierHashes) {
      nullifiers.push({
        ...
        nullifier: ByteUtils.formatToByteLength(
          nullifierHash,
          ByteLength.UINT_256
        ),
  ...
  static async processAccumulatorEvent(
    ...
    try {
      const { update, accumulatorNumber, startPosition } =
        getAccumulatorEventObject(args);
      ...
      const commitments =
        "length" in unformattedCommitments ? unformattedCommitments : [];
      // 'transactions' is potentially parsed as an object.
      const transactions =
        "length" in unformattedTransactions ? unformattedTransactions : [];
      ...
      const commitmentCiphertext =
        "length" in unformattedCommitmentCiphertext
          ? unformattedCommitmentCiphertext
          : [];
      // 'treasuryFees' is potentially parsed as an object.
      const treasuryFees =
        "length" in unformattedTreasuryFees ? unformattedTreasuryFees : [];

      const treasuryFeeMap: {
        [tokenHash: string]: { shield: bigint; unshield: bigint };
      } = {};
      ...
      for (const { tokenID, fee } of treasuryFees) {
        ...
        const shield = shields.find(({ preimage }) => {
          const shieldTokenHash =
            extractTokenHashFromCommitmentPreImageV3(preimage);
        ...
        if (unshieldValue < shieldValue) {
          unshieldFeePortion =
            (unshieldValue * fee) / (unshieldValue + shieldValue);
          shieldFeePortion = fee - unshieldFeePortion;
        } else {
          shieldFeePortion =
            (shieldValue * fee) / (shieldValue + unshieldValue);
      ...
      for (let i = 0; i < transactions.length; i += 1) {
        ...
        const commitmentsEndIndex =
          commitmentsStartIndex + Number(commitmentsCount);
        ...
        await eventsRailgunTransactionsV3Listener(txidVersion, [
          railgunTransaction,
        ]);
        ...
        if (hasUnshield) {
          ...
          const totalUnshieldValuesForToken = transactions.reduce(
            (acc, curr) => {
              return acc + curr.unshieldPreimage.value;
            },
            0n
          );
          const unshieldTokenHash =
            extractTokenHashFromCommitmentPreImageV3(unshieldPreimage);
          ...
          const unshieldFee =
            totalUnshieldValuesForToken > 0n
              ? (treasuryFeeMap[unshieldTokenHash].unshield *
      ...
      const err = new Error("Failed to process V3 accumulator update event", {
        cause,
      });
...
const getAccumulatorEventObject = (
  args: Result
