...
export class RailgunSmartWalletContract extends EventEmitter {
  ...
  private async handleNullifiedEvent(
    ...
    const { treeNumber, nullifier } =
      event.args.toObject() as NullifiedEvent.OutputObject;

    const nullifierDecoded: string[] = recursivelyDecodeResult(
      nullifier as Result
    );
  ...
  private async handleShieldEvent(
    ...
    const commitmentsDecoded: CommitmentPreimageStructOutput[] =
      recursivelyDecodeResult(commitments as Result);
    const shieldCiphertextDecoded: ShieldCiphertextStructOutput[] =
      recursivelyDecodeResult(shieldCiphertext as Result);
  ...
  private async handleTransactEvent(
    ...
    const ciphertextDecoded: CommitmentCiphertextStructOutput[] =
      recursivelyDecodeResult(ciphertext as Result);
  ...
  private async handleUnshieldEvent(
    ...
    const { to, token, amount, fee } =
      event.args.toObject() as UnshieldEvent.OutputObject;

    const tokenDecoded: TokenDataStructOutput = recursivelyDecodeResult(
  ...
  async setTreeUpdateListeners(
    ...
    const nullifiedTopic = this.contract
      .getEvent("Nullified")
      .getFragment().topicHash;
    const shieldTopic = this.contract
      .getEvent("Shield")
      .getFragment().topicHash;
    const transactTopic = this.contract
      .getEvent("Transact")
      .getFragment().topicHash;
    const unshieldTopic = this.contract
      .getEvent("Unshield")
      .getFragment().topicHash;
  ...
  private async scanAllEvents(
    ...
    try {
      ...
      if (
        retryCount < MAX_SCAN_RETRIES &&
        cause.message === SCAN_TIMEOUT_ERROR_MESSAGE
      ) {
        ...
        EngineDebug.log(
          `[Chain ${this.chain.type}:${
            this.chain.chainId
          }]: Scan query error at block ${startBlock}. Retrying ${
            MAX_SCAN_RETRIES - retry
          } times.`
        );
        EngineDebug.error(err);
  ...
  private static getShieldPreMar23EventFilter(): TypedDeferredTopicFilter<ShieldEvent_LegacyShield_PreMar23.Event> {
    ...
    const shieldPreMar23EventFragment =
      ifaceLegacyShieldPreMar23.getEvent("Shield");
    ...
    const legacyPreMar23EventFilterShield: TypedDeferredTopicFilter<TypedContractEvent> =
      {
        getTopicFilter: async () =>
          ifaceLegacyShieldPreMar23.encodeFilterTopics("Shield", []),
  ...
  private static filterEventsByTopic<Event extends TypedContractEvent>(
    ...
    return events.filter(
      (event) =>
        ...
        eventFilter.fragment.topicHash === event.topics[0]
    );
  }
  ...
  async getHistoricalEvents(
    ...
    const engineV3StartBlockNumber =
      RailgunSmartWalletContract.getEngineV2StartBlockNumber(this.chain);
    const engineV3ShieldEventUpdate030923BlockNumber =
      RailgunSmartWalletContract.getEngineV2ShieldEventUpdate030923BlockNumber(
        this.chain
      );
    ...
    const legacyEventsContract = this
      .contract as unknown as RailgunLogic_LegacyEvents;
    const legacyEventFilterNullifiers =
      legacyEventsContract.filters.Nullifiers();
    ...
    while (currentStartBlock < latestBlock) {
      ...
      const withinLegacyEventRange =
        currentStartBlock <= engineV3StartBlockNumber;
      ...
      const withinNewV3ShieldEventRange =
        endBlock >= engineV3ShieldEventUpdate030923BlockNumber;
      ...
      if (withinV3EventRange) {
        if (withinLegacyV3ShieldEventRange) {
          ...
          const eventsShieldLegacyV3 =
            RailgunSmartWalletContract.filterEventsByTopic(
              allEvents,
        ...
        if (withinNewV3ShieldEventRange) {
          ...
          await V2Events.processShieldEvents(
            txidVersion,
            eventsCommitmentListener,
            eventsShield
          );
        ...
        await Promise.all([
          V2Events.processNullifiedEvents(
            txidVersion,
            eventsNullifierListener,
            eventsNullifiers
          ),
          V2Events.processUnshieldEvents(
            txidVersion,
            eventsUnshieldListener,
            eventsUnshield
          ),
          V2Events.processTransactEvents(
            txidVersion,
            eventsCommitmentListener,
            eventsTransact
          ),
      ...
      if (withinLegacyEventRange) {
        const legacyEventsNullifiers =
          RailgunSmartWalletContract.filterEventsByTopic(
            allEvents,
            legacyEventFilterNullifiers
          );
        const legacyEventsGeneratedCommitmentBatch =
          RailgunSmartWalletContract.filterEventsByTopic(
            allEvents,
            legacyEventFilterGeneratedCommitmentBatch
          );
        const legacyEventsEncryptedCommitmentBatch =
          RailgunSmartWalletContract.filterEventsByTopic(
            allEvents,
      ...
      const nextStartBlockFromCurrentBlock =
      ...
      if (
        nextStartBlockFromLatestValidMerkletreeEntry >
        nextStartBlockFromCurrentBlock
      ) {
        ...
        EngineDebug.log(
          `[Chain ${this.chain.type}:${this.chain.chainId}]: Skipping ${
            nextStartBlockFromCurrentBlock -
            nextStartBlockFromLatestValidMerkletreeEntry
    ...
    EngineDebug.log(
      `[Chain ${this.chain.type}:${this.chain.chainId}]: Finished historical event scan`
    );
  }
  ...
  async hashCommitment(commitment: CommitmentPreimageStruct): Promise<string> {
    return this.contract.hashCommitment({
      ...commitment,
      npk: ByteUtils.formatToByteLength(
        commitment.npk,
        ByteLength.UINT_256,
        true
      ),
