...
describe("x-cha-cha-20", () => {
  it("Should test the correctness of encrypt/decrypt with XChaCha20Poly1305", () => {
    ...
    expect(XChaCha20.decryptChaCha20Poly1305(ciphertext, key)).to.deep.equal(
      plaintext
    );
  });
  ...
  it("Should reject invalid tag for XChaCha20Poly1305", () => {
    ...
    ciphertext.bundle = `${ciphertext.bundle.substring(
      0,
      ciphertext.bundle.length - 8
    )}ffffffff`;

    // Test decryption returns correct plaintext array
    expect(() => XChaCha20.decryptChaCha20Poly1305(ciphertext, key)).to.throw(
      "invalid tag"
    );
  });
  ...
  it("Should encrypt and decrypt XChaCha20 data", () => {
    const plaintext = `${ByteUtils.randomHex(32)}${ByteUtils.randomHex(
      32
    )}${ByteUtils.randomHex(32)}${ByteUtils.randomHex(32)}${ByteUtils.randomHex(
      32
    )}`;
    const viewingPrivateKey =
      71304128950017749550555748140089622855554443655032326837948344032235540545721n;
