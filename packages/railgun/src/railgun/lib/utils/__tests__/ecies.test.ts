...
describe("ecies", () => {
  it("Should encrypt and decrypt data using shared keys", async () => {
    ...
    const sharedKey = await getSharedSymmetricKeyLegacy(
      privateKey2,
      publicKey1
    );
    ...
    const sharedKeyAlternate = await getSharedSymmetricKeyLegacy(
      privateKey1,
      publicKey2
    );
    ...
    const decrypted = tryDecryptJSONDataWithSharedKey(
      encryptedData,
      sharedKeyAlternate
    );
