...
function getCommitmentBlindingKeyLegacy(
  random: string,
  senderRandom: string
): bigint {
  ...
  const commitmentBlindingKey =
    ByteUtils.hexToBigInt(random) ^ ByteUtils.hexToBigInt(senderRandom);

  const commitmentBlindingKeyHex = ByteUtils.nToHex(
  ...
  // receiver
  const commitmentBlindingKeyNormalized = normalizeRandomLegacy(
    commitmentBlindingKeyHex
  );
...
function unblindNoteKeyLegacy(
  ...
  try {
    const commitmentBlindingKey = getCommitmentBlindingKeyLegacy(
      random,
      senderRandom
    );
...
async function getSharedSymmetricKeyLegacy(
  ...
  try {
    ...
    return scalarMultiplyWasmFallbackToJavascript(
