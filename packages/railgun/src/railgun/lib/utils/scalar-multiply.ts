...
const initCurve25519Wasm = (): Promise<void> => {
  try {
    ...
    return typeof initCurve25519wasm === "function"
      ? initCurve25519wasm()
      : Promise.resolve();
  } catch (cause) {
    ...
    EngineDebug.log(
      "curve25519-scalarmult-wasm init failed: Fallback to JavaScript"
    );
...
export const scalarMultiplyWasmFallbackToJavascript = (
  ...
  try {
    ...
    if (!(cause instanceof Error)) {
      throw new Error(
        "Non-error thrown from scalarMultiplyWasmFallbackToJavascript",
        { cause }
      );
    }
    ...
    EngineDebug.log(
      "curve25519-scalarmult-wasm scalarMultiply failed: Fallback to JavaScript"
    );
    EngineDebug.error(cause);
