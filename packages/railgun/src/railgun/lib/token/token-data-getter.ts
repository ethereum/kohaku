...
export class TokenDataGetter {
  ...
  async getTokenDataFromHash(
    ...
    const formatted = ByteUtils.formatToByteLength(
      tokenHash,
      ByteLength.UINT_256
    );
    ...
    const tokenDataNFT = await this.getNFTTokenData(
      txidVersion,
      chain,
      tokenHash
    );
  ...
  private async cacheNFTTokenData(
    tokenHash: string,
    tokenData: TokenData
  ): Promise<void> {
    await this.db.put(
  ...
  async getCachedNFTTokenData(tokenHash: string): Promise<Optional<TokenData>> {
    try {
      ...
      if (!(cause instanceof Error)) {
        throw new Error("Non-error thrown from getCachedNFTTokenData", {
          cause,
        });
      }
