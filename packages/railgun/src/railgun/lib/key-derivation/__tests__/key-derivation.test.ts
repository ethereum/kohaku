...
const VECTORS = [
  {
    ...
    spendingKeyPair: {
      privateKey: new Uint8Array([
        103, 215, 209, 157, 0, 230, 227, 179, 81, 127, 230, 138, 196, 101, 5,
        221, 32, 125, 246, 232, 254, 58, 160, 107, 163, 250, 206, 53, 46, 117,
        153, 239,
      ]),
      pubkey: [
    ...
    viewingKeyPair: {
      ...
      pubkey: new Uint8Array([
        13, 235, 247, 125, 142, 148, 54, 252, 7, 160, 220, 63, 232, 189, 144,
        194, 245, 146, 160, 140, 171, 141, 190, 95, 151, 42, 71, 131, 70, 92,
        214, 212,
      ]),
    },
    nullifyingKey:
  ...
  },
  {
    ...
    spendingKeyPair: {
      privateKey: new Uint8Array([
        52, 40, 207, 201, 57, 50, 3, 40, 80, 17, 116, 164, 231, 110, 134, 145,
        151, 255, 200, 148, 181, 141, 191, 77, 14, 149, 60, 72, 77, 102, 203,
        94,
      ]),
      pubkey: [
    ...
    viewingKeyPair: {
      ...
      pubkey: new Uint8Array([
        188, 10, 133, 20, 54, 28, 82, 39, 129, 118, 54, 192, 105, 143, 30, 183,
        217, 77, 82, 240, 122, 203, 88, 224, 107, 241, 219, 145, 159, 230, 69,
        20,
      ]),
    },
    nullifyingKey:
  ...
  },
  {
    ...
    spendingKeyPair: {
      privateKey: new Uint8Array([
        174, 161, 99, 229, 87, 84, 149, 94, 180, 249, 130, 131, 138, 60, 244,
        236, 172, 83, 232, 9, 167, 168, 25, 67, 167, 236, 115, 134, 66, 111,
        157, 93,
      ]),
      pubkey: [
    ...
    viewingKeyPair: {
      ...
      pubkey: new Uint8Array([
        28, 112, 24, 121, 34, 161, 22, 10, 20, 98, 221, 72, 216, 131, 91, 146,
        55, 237, 168, 255, 121, 6, 217, 124, 152, 150, 232, 196, 138, 161, 179,
        243,
      ]),
    },
    nullifyingKey:
  ...
  it("Should derive spending keys", async () => {
    ...
    await Promise.all(
      VECTORS.map(async (vector) => {
        ...
        expect(node.derive(vector.path).getSpendingKeyPair()).to.deep.equal(
          vector.spendingKeyPair
        );
      })
    );
  });
  ...
  it("Should derive viewing keys", async () => {
    ...
    await Promise.all(
      VECTORS.map(async (vector) => {
        ...
        expect(
          await node.derive(vector.path).getViewingKeyPair()
        ).to.deep.equal(vector.viewingKeyPair);
      })
    );
  });
