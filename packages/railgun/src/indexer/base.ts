...
export const createRailgunIndexer: CreateRailgunIndexerFn = async ({ network, provider, startBlock, storage }) => {
    const accounts: RailgunAccount[] = [];
    const { trees, saveTrees, getCurrentBlock, setEndBlock } = await createIndexerStorage(storage, { startBlock });
    const getTrees = () => trees;
    const processLog = await makeProcessLog({ getTrees, accounts });
    const { sync } = await createRpcSync({ network, provider, getCurrentBlock, accounts, processLog, getTrees, saveTrees, setEndBlock });
