...
export const loadCachedMerkleTrees = async (cachedTrees: CachedMerkleTrees) => {
  ...
  for (let i = 0; i < cachedTrees.length; i++) {
    ...
    merkleTree.tree = cachedTrees[i]!.tree.map((level) =>
...
export const serializeMerkleTrees = (
  trees: MerkleTree[]
): CachedMerkleTrees => {
  const merkleTrees = [];
  ...
  for (const tree of trees) {
    merkleTrees.push({
      tree: tree.tree.map((level) =>
        level.map((leaf) => ByteUtils.hexlify(leaf, true))
      ),
      nullifiers: tree.nullifiers.map((nullifier) =>
        ByteUtils.hexlify(nullifier, true)
      ),
    });
  }
...
export const createIndexerStorage = async (
  ...
  const { load, save } = createBaseStorage<IndexerCache, IndexerCacheD>(layer, {
    async parse(
      { merkleTrees, endBlock } = { merkleTrees: [], endBlock: startBlock || 0 }
    ) {
      return {
    ...
    async serialize({ trees, endBlock }) {
      return {
        merkleTrees: await serializeMerkleTrees(trees),
        endBlock: endBlock,
      };
    },
  });
