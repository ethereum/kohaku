...
export type RpcSyncFn = (params?: {
  fromBlock?: number;
  toBlock?: number;
  logProgress?: boolean;
}) => Promise<void>;
export type RpcGetLogsFn = (
  fromBlock: number,
  toBlock: number
...
function isRangeErr(e: any) {
  return (
    ...
    /failed to resolve block range/i.test(
      String(
        e?.error?.message ||
          e?.message ||
          e?.info?.responseBody ||
...
export const createRpcSync: (
  context: RpcSyncContext
) => Promise<RpcSync> = async ({
  provider,
  processLog,
  getTrees,
  getCurrentBlock,
  ...
  const getLogs: RpcGetLogsFn = async function* (startBlock, endBlock) {
    ...
    while (fromBlock <= endBlock) {
      ...
      try {
        ...
        console.log(
          "[sync]: yielding logs (duration: " + formatDuration(duration) + ")"
        );
        yield { logs, toBlock };
  ...
  const sync: RpcSyncFn = async ({ fromBlock, toBlock, logProgress } = {}) => {
    ...
    for await (const batch of bufferedLogs) {
      ...
      console.log(
        "Processing batch of logs (size: ",
        batch.length,
        ", entries: ",
        batch.reduce((acc, { logs }) => acc + logs.length, 0),
        ")"
      );
      ...
      if (logProgress) {
        ...
        const estimatedTimeRemaining =
          (elapsedTime * (endBlock - lastBlock)) / (lastBlock - startBlock);
