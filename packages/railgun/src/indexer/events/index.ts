...
export type RailgunLogEvent =
  | { name: "Shield"; args: ShieldEvent }
  | { name: "Transact"; args: TransactEvent }
  | { name: "Nullified"; args: NullifiedEvent };

export const makeProcessLog = async ({
  getTrees,
  accounts,
}: ProcessLogContext): Promise<ProcessLogFn> => {
  ...
  return async ({ log, skipMerkleTree = false }) => {
    ...
    await match(event)
      .with({ name: "Shield" }, (event) =>
        handleShieldEvent(event.args, skipMerkleTree)
      )
      .with({ name: "Transact" }, (event) =>
        handleTransactEvent(event.args, skipMerkleTree)
      )
      .with({ name: "Nullified" }, (event) =>
