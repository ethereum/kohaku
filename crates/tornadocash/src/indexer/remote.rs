...
//     indexer::syncer::{Syncer, SyncerError},
//     provider::Pool,
// };
...
//         from_block: u64,
//         to_block: u64,
//     ) -> Result<Vec<Deposit>, SyncerError> {
//         let pool = Pool::from_address(contract).ok_or(RemoteSyncerError::UnknownPool(contract))?;
//         Ok(self
//             .deposits(&pool)
//             .await?
...
//         from_block: u64,
//         to_block: u64,
//     ) -> Result<Vec<Withdrawal>, SyncerError> {
//         let pool = Pool::from_address(contract).ok_or(RemoteSyncerError::UnknownPool(contract))?;
//         Ok(self
//             .nullifiers(&pool)
//             .await?
...
// impl RemoteSyncer {
//     async fn deposits(&self, pool: &Pool) -> Result<Vec<Deposit>, RemoteSyncerError> {
...
//     async fn nullifiers(&self, pool: &Pool) -> Result<Vec<Withdrawal>, RemoteSyncerError> {
//         let nullifiers = self
//             .client
//             .get(&self.withdrawals_url(&pool))
//             .await?
//             .text()?;
//         let nullifiers: Vec<Withdrawal> = nullifiers
//             .lines()
//             .filter(|line| !line.is_empty())
//             .map(|line| serde_json::from_str(line))
//             .collect::<Result<_, _>>()?;
...
//     fn withdrawals_url(&self, pool: &Pool) -> String {
//         format!(
//             "{}/{}_{}_{}_withdrawals.ndjson",
//             self.base_url,
//             pool.chain_id,
//             pool.symbol(),
//             pool.amount()
