...
[Walletbeat](https://beta.walletbeat.eth.limo/) aims to provide a comprehensive overview of the Ethereum wallet ecosystem.
...
## Many accounts, many you

Or maybe more realistic, different accounts for different things.
The process to create a new account, top it up, and get going should be as seamless as possible.

A good first step would be to have a "create new" button, when connecting to a dApp.
In addition to a privacy-friendly way to top up the your newly created account.
...
[Nimbus Verified Proxy](https://github.com/status-im/nimbus-eth1/tree/master/nimbus_verified_proxy) is another light client proxy developed by [Status](https://status.im/) that verifies Ethereum RPC responses using Merkle proofs against the consensus light client's state, without requiring a full node.
