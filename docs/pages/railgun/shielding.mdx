...
```ts twoslash [shield-eth.ts] filename="shield-eth.ts"
// @noErrors
...
const tx = await account.shield(chainId, WETH, VALUE); // [!code focus]
...
const tx = await account.transfer(chainId, WETH, recipientZkAddress, amount); // [!code focus]
