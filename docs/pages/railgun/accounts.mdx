...
// @noErrors
import { createRailgunAccount, createRailgunIndexer, ViemProviderAdapter } from '@kohaku-eth/railgun'
...
import { createRailgunAccount, ViemProviderAdapter } from '@kohaku-eth/railgun';
import { createPublicClient, http } from 'viem';
...
export const account = await createRailgunAccount({
  credentials: { mnemonic, accountIndex: 0 },
  indexer,
});
```
