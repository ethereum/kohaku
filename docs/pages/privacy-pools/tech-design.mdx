...
![native-account-initialization.svg](/privacy-pools/native-account-initialization.svg)
...
![legacy-account-import.svg](/privacy-pools/legacy-account-import.svg)
...
![account-loading.svg](/privacy-pools/account-loading.svg)
...
![deposit.svg](/privacy-pools/deposit.svg)
...
![withdrawal-1.svg](/privacy-pools/withdrawal-1.svg)

<br/>

![withdrawal-2.svg](/privacy-pools/withdrawal-2.svg)
...
![ragequit.svg](/privacy-pools/ragequit.svg)
...
- Sync write-through:
    - Fetch logs once → discover notes in memory → batch write new/updated records → update `last_synced_block`.
- Startup:
    - Load accounts + notes from local storage → quick UI → run `syncAll()` to catch up.
...
![security-model.svg](/privacy-pools/security-model.svg)
