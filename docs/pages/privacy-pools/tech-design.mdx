...
[Privacy Methods Specification](/privacy-pools/privacy-methods-specification)
...
sequenceDiagram
    ...
    C->>C: Select for desiredNet → noteCount1, gross≈desiredNet
    C->>R: POST /batch/quote { netAmount=desiredNet, batchSize=noteCount1 }
    R-->>C: relayFeeBPS, grossNeeded, estimatedGas
    C->>C: Select for grossNeeded → noteCount2, grossFromPlan
